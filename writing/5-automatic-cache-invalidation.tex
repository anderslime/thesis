...
Based on the type and id of the changed entity, Smache updates affected cached objects in four steps:

\begin{enumerate}
  \item Query the IDG using \verb$lookup_cached_object$ with the updated entity id to find instance dependencies
  \item Query the DDG using \verb$lookup_lazy_dependencies$ with the update entity type to find lazy dependencies
  \item Evaluate the procedures for lazy dependencies, which returns a set of entities
  \item Query the IDG using \verb$lookup_cached_object$ with the set of entities from the lazy dependency procedures
  \item Invalidate cached objects from instance and lazy dependencies using timestamp invalidation
\end{enumerate}
