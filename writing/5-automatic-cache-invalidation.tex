...
% - The representation of a cached object
%   => This is where we explain how we do timestamp invalidation
%   => Important feature: possible to do transaction between timestamp and value
%   => Solution: represent cached object as value + update_num + last_update_timestamp
%   => Else if possible to do transactions across keys
%   => Request same as in cachable functions, where the freshness is evaluated as with timestamp invalidation
...
% - Architecture
%   - Timestamp Invalidation as part integrated part of the cache
%     => Easy to shard the database
% - Discussions:
%   - How do we represent timestamp invalidation?
%   => As part of the cached value
%   => Easy to shard the cache database (and allow to scale horizontally)

% Architecture
% - Architecture:
%   - Describe as extension of the one described for basic caching
%   - Additions:
%     - IDG (as part of cache? or externally?)
%     - DDG (in application memory)
%     - Timestamps as part of cache?
